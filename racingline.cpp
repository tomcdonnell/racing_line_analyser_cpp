...
bool racingLine::fitToCorner(line eL, line xL, rec2vector apex)
{
   ...
   enum {both, entry, exit, neither} YeqMxPlusC;
        if ( eL.YeqMxPlusC &&  xL.YeqMxPlusC) YeqMxPlusC = both;
   else if ( eL.YeqMxPlusC && !xL.YeqMxPlusC) YeqMxPlusC = entry;
   else if (!eL.YeqMxPlusC &&  xL.YeqMxPlusC) YeqMxPlusC = exit;
   else                                       YeqMxPlusC = neither;
   ...
   if (w == 0)
   {
      switch (YeqMxPlusC)
      {
       ...
       case neither:
         ...
         eL.YeqMxPlusC =  true; xL.YeqMxPlusC = true;
         swap(apex.x, apex.y);
       ...
       case entry:
         ...
         eL.YeqMxPlusC = false; xL.YeqMxPlusC = true;
         swap(apex.x, apex.y);
         f = -f + pi / 2; // mirror angle about y = x line
 ...
 *
 */
void racingLine::plot(COLORREF c)
{
   ...
   for (double t = tE; t <= tX; t += dt)
   {
          ...
          && -1.0 <= newPos.y && newPos.y <= 1.0)
      {
         ...
         drawArrow(oldPos.x, oldPos.y, f, 0.07 * A, 0.03, BLACK);
         drawArrow(newPos.x, newPos.y, f, 0.07 * A, 0.03, c);
         ...
         // scaling
         newV = vectMultScalar(newV, 0.1);

         // draw velocity vector arrow
         drawArrow(oldPos, convToPol(oldV), 0.03, BLACK);
         drawArrow(newPos, convToPol(newV), 0.03, c);

         // draw racing line segment
         drawLine(oldPos, newPos, c);
      }
...
bool racingLine::fitToCornerWzeroBothYeqMxPlusC(line eL, line xL, rec2vector apex)
{
   ...
   else
   {
      ...
      if (apex.y > intersection(eL, xL).y) bL.set(0.0,  1.0, true); // top    (y = 0.0x + 1.0)
      else                                 bL.set(0.0, -1.0, true); // bottom (y = 0.0x - 1.0)
...
int plotRacingLines(racingLine rl[5], const int n)
{
   ...
   for (double t = 0; t <= tEnd; t += dt)
   {
      for (i = 0; i < n; ++i)
      {
         ...
         {
            // draw racing line segment
            drawLine(oldPos[i], newPos[i], getColor(i));
         }
